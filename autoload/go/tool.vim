...
function! go#tool#Files(...) abort
  ...
  let [l:out, l:err] = go#util#ExecInDir([go#path#GoCmd(), 'list', '-tags', go#config#BuildTags(), '-f', l:combined])
  return split(l:out, '\n')
endfunction
...
function! go#tool#Deps() abort
  ...
  let [l:out, l:err] = go#util#ExecInDir([go#path#GoCmd(), 'list', '-tags', go#config#BuildTags(), '-f', l:format])
  return split(l:out, '\n')
endfunction
...
function! go#tool#Imports() abort
  ...
  for package_path in split(out, '\n')
    let [l:out, l:err] = go#util#ExecInDir([go#path#GoCmd(), 'list', '-tags', go#config#BuildTags(), '-f', '{{.Name}}', l:package_path])
    if l:err != 0
      echo out
      return imports
    endif
...
function! go#tool#PackageName() abort
  let [l:out, l:err] = go#util#ExecInDir([go#path#GoCmd(), 'list', '-tags', go#config#BuildTags(), '-f', '{{.Name}}'])
  if l:err != 0
      return -1
  endif
...
function! go#tool#Exists(importpath) abort
    let [l:out, l:err] = go#util#ExecInDir([go#path#GoCmd(), 'list', a:importpath])
    if l:err != 0
        return -1
    endif
